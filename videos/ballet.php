<?php
        //session_start();
        //include('../cabecera/menu.php');
        include('../cabecera/tipo.php');
        include('../aVideos.php');
        ?>
        <section class='section-agents section-t8'>
            <div class='container'>
                <h1 class='text-center'>Videos de Ballet</h1>

                <style>
                    #video {
                        width: 98%;
                        height: 98%;
                    }
                </style>

                <div class='row'>
                
                    <div class='col-md-6 col-sm-12 col-xs-12 embed-responsive embed-responsive-16by9'>
                       <iframe id='video' class='embed-responsive-item' src='https://www.youtube.com/embed/BoKx7BTb0lI' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>
                    </div>

                    <div class='col-md-6 col-sm-12 col-xs-12 embed-responsive embed-responsive-16by9'>
                       <iframe id='video' class='embed-responsive-item' src='' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>
                    </div>

                    <div class='col-md-6 col-sm-12 col-xs-12 embed-responsive embed-responsive-16by9'>
                       <iframe id='video' class='embed-responsive-item' src='' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>
                    </div>

                    <div class='col-md-6 col-sm-12 col-xs-12 embed-responsive embed-responsive-16by9'>
                       <iframe id='video' class='embed-responsive-item' src='' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>
                    </div>

                    <div class='col-md-6 col-sm-12 col-xs-12 embed-responsive embed-responsive-16by9'>
                       <iframe id='video' class='embed-responsive-item' src='' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>
                    </div>

                    <div class='col-md-6 col-sm-12 col-xs-12 embed-responsive embed-responsive-16by9'>
                       <iframe id='video' class='embed-responsive-item' src='' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>
                    </div>
                
                </div>

                <br><br>

                <div class='col-md-12'>
                    ballet
                </div>

            </div>
        </section>
        <?php include('../cabecera/pie.php'); ?>